...
class Contact extends Component {
    ...
    validate(firstname, lastname, telnum, email){
        ...
        if(this.state.touched.firstname && firstname.length <3)
            ...
            errors.firstname = 'First name should be no longer than 10 characters';

        if(this.state.touched.lastname && lastname.length <3)
            errors.lastname = 'Last name should be longer than 2 characters';
        else if (this.state.touched.lastname && lastname.length >10)
            errors.lastname = 'Last name should be no longer than 10 characters';

        const reg = /^\d+$/;
        if(this.state.touched.telnum && !reg.test(telnum))
            errors.telnum = 'Tel. number should contain numbers only.'
        const ereg = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
        if(this.state.touched.email && !ereg.test(email))
            errors.email = 'Email should be in the example@example.example format.'
